...
class ContrastiveConfig:
    n_samples: int = 5000  # Total samples to use from FashionMNIST
...
# -----------------------------------------------------
# Load and subsample FashionMNIST
# -----------------------------------------------------
def load_fashion_mnist_imbalanced(config, seed=42):
    """
    Load FashionMNIST and create highly imbalanced subset
    """
    np.random.seed(seed)
    random.seed(seed)
...
def visualize_rare_class_embeddings(X_test, labels_test, encoder_weighted, encoder_unweighted, 
    ...
    plt.tight_layout()
    plt.savefig('results/fashion_mnist_rare_class_embeddings.pdf', dpi=150, bbox_inches='tight')
    plt.show()
...
def visualize_comparison(results_weighted, results_unweighted, class_sizes):
    ...
    plt.savefig('results/fashion_mnist_comparison_results.pdf', dpi=300, bbox_inches='tight')
    plt.show()
...
def main():
    ...
    print("-"*60)
    X_train_img, labels_train, X_test_img, labels_test, class_sizes = load_fashion_mnist_imbalanced(config)
